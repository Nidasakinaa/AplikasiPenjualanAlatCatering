...
class Checkout extends MY_Controller
{
    ...
    public function index($input = null)
    {
        ...
        $data['reserveation_date'] = (object) $this->input->post(null, true);
        ...
        if (!$data['cart']) { //apabila tidak ada barang di keranjang atau kosong keranjangnya
    ...
    public function create()
    {
        ...
        // Menyiapkan insert table orders
            ...
            'id_user'   => $this->id,
            'date'      => date('Y-m-d'),
            'invoice'   => $this->id . date('YmdHis'), //no invoice berdasarkan waktu checkout
            'total'     => $total,
            'name'      => $input->name,
            ...
            'dt_end'    => $input->dt_end,
            'dt_interval'    => $input->dt_interval, //Jumlah hari penyewaan 
            'status'    => 'waiting'
        ];
        ...
        if ($id_orders = $this->checkout->create($data)) { 
            ...
            $this->db->delete('cart', ['id_user' => $this->id]);    // setelah co maka Hapus cart user sekarang
